@php
    ...
    $difficultyLabel = __('courses.difficulty_' . $course->difficulty);
@endphp
<div class="course-card group flex flex-col" data-course-item data-course-title="{{ strtolower($course->title) }}">
    <div class="course-card-cover">
        ...
        <span class="course-card-price">{{ $course->price_mad }} {{ __('common.mad') }}</span>
    </div>

    <div class="flex flex-1 flex-col p-4">
        <div class="mb-2 flex items-center gap-2">
            <span class="rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide {{ $difficultyStyle }}">{{ $difficultyLabel }}</span>
            @if($avgRating !== null)
        ...
        <div class="mt-3 flex items-center gap-4 text-xs text-(--color-text-secondary)">
            ...
            <span class="flex items-center gap-1.5">
                ...
                {{ trans_choice('courses.students_count', $studentsCount, ['count' => $studentsCount]) }}
            </span>
        </div>
